...
runner.test('Энергия в целом уменьшается в процессе обучения', async () => {
  const rbm = new TestRBM(6, 4);
  const data = generateTestData(20, 6);
  ...
  // Энергия может флуктуировать, но в целом должна уменьшаться
  ...
  console.log(`  Уменьшение энергии: ${energyReduction.toFixed(4)}`);
  
  // Допускаем небольшое увеличение энергии из-за стохастичности
  runner.assert(energyReduction > -2.0, 'Энергия не должна сильно увеличиваться');
});
...
runner.test('Алгоритм стабилен при многократном запуске', async () => {
  ...
  runner.assert(maxDeviation < 0.3, 'Результаты должны быть стабильными между запусками');
});
...
runner.test('Температура уменьшается монотонно', async () => {
  ...
  // Модифицируем алгоритм для отслеживания температуры
  const initialTemperature = 2.0;
  const finalTemperature = 0.1;
...
<parameter name="todos">[{"id": "1", "content": "Create tests for simulated annealing algorithm", "status": "completed"}, {"id": "2", "content": "Test energy calculation correctness", "status": "completed"}, {"id": "3", "content": "Test acceptance rate behavior", "status": "completed"}, {"id": "4", "content": "Test weight updates during training", "status": "completed"}, {"id": "5", "content": "Run all tests and verify results", "status": "in_progress"}]